class BMP {
  ...
  getColorByPosInt(posInt: number): [number, number, number] {
    const pos = posInt * 3 + this.bfOffBits
    const b = this.buffer.readUint8(pos)
    const g = this.buffer.readUint8(pos + 1)
    const r = this.buffer.readUint8(pos + 2)
    return [r, g, b]
  }
  ...
  calPos(posInt: number): [number, number] {
    const y = Math.floor(posInt / this.biWidth)
    const x = posInt - y * this.biWidth
    return [x, y]
  }
}
