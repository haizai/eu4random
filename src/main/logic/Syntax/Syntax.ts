...
class Syntax {
  ...
  private parseKey(): null | string {
    const m = this.match(Syntax.keyEqualSpaceReg)
    if (m && m.length >= 2) {
      const keyEqualSpace = m[0]
      const key = m[1]
  ...
  private parseRoot() {
    ...
    while (true) {
      ...
      if (lastOffset == this.offset) {
        throw new Error(
          `未解析的字符: ${this.str[this.offset]} ${this.str[this.offset].charCodeAt(0)}`
        )
      }
    }
  }
  private parseValue(): SyntaxValue {
    const m = this.match(Syntax.leftCurlyReg)
    if (m) {
      this.offset += m[0].length
      const ret: SyntaxValue = []
      while (true) {
        const lastOffset = this.offset
        const skv = this.parseItem()
        this.parseComment()
        ...
        if (lastOffset == this.offset) {
          throw new Error(
            `parse char Error: ${this.str[this.offset]} ${this.str[this.offset].charCodeAt(0)}`
          )
        }
  ...
  private parseSimpleValue(): null | string {
    const m = this.match(Syntax.simpleValueSpaceReg)
    if (m && m.length >= 2) {
      const simpleValueSpace = m[0]
      const simpleValue = m[1]
  ...
  private IsParseRightCurly() {
    const m = this.match(Syntax.rightCurlyReg)
    if (m && m.length >= 1) {
      this.offset += m[0].length
      return true
    }
...
export { Syntax }
export type { SyntaxKeyValue, SyntaxItem, SyntaxValue }
